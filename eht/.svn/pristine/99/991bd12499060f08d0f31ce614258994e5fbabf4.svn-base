...
$.extend($.validator, {
	...
	prototype: {
		...
		elements: function() {
			...
			return $(':input',this.currentForm) .not(":submit, :reset, :image, [disabled]") .not( this.settings.ignore ) 
			.filter(function() { !this.name && validator.settings.debug && window.console && console.error( "%o has no name assigned", this);
	...
	methods: {
		...
		remote: function(value, element, param) {
			...
			// [&& element.defaultValue !==value] add by Chang Fei.
			if (param.url.indexOf("?") > 0) {
				param.url += "&_rmr_=" + Math.random();
			} else {
				param.url += "?_rmr_=" + Math.random();
			}
			//alert(param.url);
		...
		regexp: function(value, element, param) {
			...
			return param.match ? result : !result;
		}
